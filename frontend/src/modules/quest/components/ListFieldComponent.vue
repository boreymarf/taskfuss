...
<template>
  <div class="list-field">
    ...
    <div v-for="(itemRef, idx) in itemRefs" :key="idx" class="flex items-start gap-2 mb-2">
      <FieldSelector :field="field.item_field" :model-value="itemRef.value"
        @update:model-value="onItemUpdate(idx, $event)" class="flex-1" />
      <button type="button" class="text-red-500 hover:text-red-700 px-2 py-1 border rounded" @click="removeItem(idx)">
        ✕
      </button>
    </div>

    <button type="button" class="mt-2 px-3 py-1 bg-blue-500 text-white rounded hover:bg-blue-600" @click="addItem">
      + Добавить
    </button>
  </div>
</template>
