...
type TaskService struct {
	...
	requirementsSnapshotsRepo *db.RequirementsSnapshotsRepository
}
...
func InitTaskService(
	...
	requirementRepo *db.RequirementsRepository,
	requirementEntryRepo *db.RequirementsEntryRepository,
	requirementSnapshotsRepo *db.RequirementsSnapshotsRepository,
) (*TaskService, error) {
	...
	repo := &TaskService{
		...
		requirementsSnapshotsRepo: requirementSnapshotsRepo,
	}
...
func (s *TaskService) CreateRequirement(requirement *dto.Requirement, task_id int64, parent_id *int64) error {
	...
	// Returns id after
	// TODO: If one fails, the data becomes incomplete, use transaction later
	modelRequirement, err := s.requirementRepo.CreateRequirement(&r)
	if err != nil {
		return err
	}

	rs := models.RequirementSnapshot{
		RequirementID: modelRequirement.ID,
		ParentID:      parent_id,
	...
	modelRequirementSnapshot, err := s.requirementsSnapshotsRepo.CreateSnapshot(&rs)
	if err := s.requirementsSnapshotsRepo.CreateSnapshot(&rs); err != nil {
		return err
	}
